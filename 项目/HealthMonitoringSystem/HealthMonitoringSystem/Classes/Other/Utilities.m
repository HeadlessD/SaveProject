...
+(UILabel *)labelWithPoint:(CGPoint)point maxWidth:(CGFloat)maxWidth text:(NSString *)text textColor:(UIColor *)color fontSize:(CGFloat)fontSize spaceLine:(BOOL)spaceLine
{
    ...
    if (spaceLine) {
        [label setLabelSpaceWithValue:text withFont:HMSFOND(fontSize)];
        label.size = [label getSpaceLabelSize:text withFont:HMSFOND(fontSize) withWidth:maxWidth];
    }else
    {
        label.text = text;
        label.size = [NSString sizeWithString:text andStrFont:HMSFOND(fontSize) andMaxSize:CGSizeMake(maxWidth, MAXFLOAT)];
    }
