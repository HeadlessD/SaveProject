...
#pragma clang diagnostic pop
    ...
    CGFloat y =boundsHeight/(self.parentView.yPoint.count+1)-self.parentView.labelsFont.pointSize*0.5-2;
    if (verticalLineCount > 0) {
        ...
        for (NSInteger idx = 0; idx < verticalLineCount; ++idx) {
            ...
            [verticalReferencePath moveToPoint:CGPointMake(point.x, y)];
    ...
    if (self.parentView.myType ==WYLineChartViewTypeTwo) {
        ...
        for (NSInteger idx = 0; idx < labelCount; ++idx) {
            ...
            label.text = [[str componentsSeparatedByString:@"-"] lastObject];
            NSString *text =[[str componentsSeparatedByString:@"-"] firstObject];
            if ([text isEqualToString:NSLocalizedString(@"524", nil)]||[text isEqualToString:NSLocalizedString(@"525", nil)]) {
