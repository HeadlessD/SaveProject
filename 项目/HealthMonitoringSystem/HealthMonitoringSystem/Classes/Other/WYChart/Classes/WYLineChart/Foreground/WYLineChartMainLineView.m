...
#pragma clang diagnostic pop
    ...
    for (WYLineChartPoint *tmpPoint in _points) {
        if (minY>tmpPoint.y) {
            minY = tmpPoint.y;
        }
    }
    ...
    if (_drawGradient && _style != kWYLineChartMainNoneLine) {
        ...
        NSArray *gradientColors = @[[_lineColor colorWithAlphaComponent:1],
                                    [_lineColor colorWithAlphaComponent:0]];
        ...
        maskLayer.frame = CGRectMake(0, -minY, self.width, self.height);
        _gradientMaskLayer = maskLayer;
        
        gradientLayer = [CAGradientLayer layer];
        [gradientLayer setFrame:CGRectMake(0, minY, self.width, self.height-minY)];//self.bounds
