...
- (CGFloat)drawableAreaHeight {
    return CGRectGetHeight(_parentView.frame) - self.xAxisViewHeight - 2;
}
...
- (CGFloat)yAxisLabelWidth {
   ...
   // CGFloat maxValueOfPoints = [[self maxValuePointsOfLinesPointSet:_parentView.yPoint] value];
    
    NSString *valueString = [NSString stringWithFormat:@"%@",[_parentView.yPoint lastObject]];
...
- (CGFloat)xAxisViewHeight {
    
    return self.xAxisLabelHeight + 10.f;
}
...
- (CGFloat)yAxisViewWidth {
    return self.yAxisLabelWidth + 8.f;
}
