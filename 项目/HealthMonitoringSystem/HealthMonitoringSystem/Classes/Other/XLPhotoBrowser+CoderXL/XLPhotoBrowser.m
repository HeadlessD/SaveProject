...
- (void)zoomingScrollView:(XLZoomingScrollView *)zoomingScrollView singleTapDetected:(UITapGestureRecognizer *)singleTap
{
    ...
    [UIView animateWithDuration:0.15 animations:^{
        weakSelf.savaImageTipLabel.alpha = 0.0;
        weakSelf.indicatorView.alpha = 0.0;
    } completion:^(BOOL finished) {
        [weakSelf.savaImageTipLabel removeFromSuperview];
...
- (void)show
{
    ...
    [[NSNotificationCenter defaultCenter]
     postNotificationName:@"HomePageStatusHiddenOrNO" object:@{@"hidden":@YES}];
    [self iniaialUI];
}
