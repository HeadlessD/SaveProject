...
- (instancetype)initWithFrame:(CGRect)frame withCountPerRowInView:(NSUInteger)ArrangeCount cellMargin:(CGFloat)cellMargin{
    if(self = [super initWithFrame: frame]){
        ...
        _margin = 12;
        _contentInsets = UIEdgeInsetsMake(10, 4, 0, 8);
...
- (NSInteger)collectionView:(UICollectionView *)collectionView numberOfItemsInSection:(NSInteger)section {
    if(_selectedPhotos.count < _maxSelectedCount) {
        if (self.changeHeight) {
            _collectionView.height =(((_selectedPhotos.count+1) + 2) / 3 ) * (_margin + _itemWH);
            self.changeHeight((((_selectedPhotos.count+1) + 2) / 3 ) * (_margin + _itemWH));
        }
    return _selectedPhotos.count + 1;
        
    }else {
        if (self.changeHeight) {
            _collectionView.height =(_selectedPhotos.count / 3 ) * (_margin + _itemWH);
            self.changeHeight((_selectedPhotos.count/ 3 ) * (_margin + _itemWH));
        }
    ...
    if (indexPath.row == _selectedPhotos.count) {
       ...
       [self pushImagePickerController];
        ...
        if (isVideo) { // 预览视频
            ...
            selectImagePickerVc.barItemTextColor = HMSThemeColor;
            selectImagePickerVc.barItemTextFont = iPhone5_5s?HMSFOND(18):HMSFOND(19);
