...
/** 最大字数 */
NSInteger const MHCommentMaxWords  = 150  ;

@interface HMSHomePageReplyInputView ()<YYTextViewDelegate>
/** 底部工具条 */
...
@end
@implementation HMSHomePageReplyInputView
- (void)dealloc
{
...
- (void)setWeiboM:(HMSWeiboModel *)weiboM
{
    _weiboM = weiboM;
    // 设置数据
    [self.avatarView sd_setImageWithURL:[NSURL URLWithString:weiboM.weibo_author_avatar] placeholderImage:[UIImage imageNamed:[weiboM.weibo_author_sex isEqualToString:@"0"]?@"defaul_manHeaderImg":@"defaul_womanHeaderImg"]];
    self.commentLabel.text = weiboM.weibo_content;
    // 设置placeholder
    self.textView.placeholderText = [NSString stringWithFormat:@"回复%@",weiboM.weibo_author_name];
    // 设置text
    // 获取缓存text
...
- (void)_setupControlView
{
    ...
    backgroundControl.backgroundColor = [UIColor colorWithWhite:0 alpha:0.2];
...
- (void)_setupBottomToolBar
{
    ...
    avatarView.image = [UIImage imageNamed:@"defaul_manHeaderImg"];
    ...
    commentLabel.font = HMSFOND(MHPxConvertPt(11.0f));
    ...
    textView.font = HMSFOND(MHPxConvertPt(13.0f));
    ...
    textView.layer.borderWidth = 1;//MHPxConvertPt(1.0f);
    textView.layer.borderColor = HMSThemeColor.CGColor;
//    textView.backgroundColor = MHColorFromHexString(@"#F5F5F5");
    ...
    NSMutableAttributedString *attributedText = [[NSMutableAttributedString alloc] initWithString:@"0/150字"];
    attributedText.yy_font = HMSFOND(MHPxConvertPt(9.0f));
...
- (void)_bottomToolBarWillChangeHeight:(CGFloat)toHeight
{
    ...
    // 不允许遮盖住 视频播放
    CGFloat maxHeight = KScreenHeight - 150 - self.keyboardHeight;
...
- (void)_textViewWordsDidChange:(YYTextView *)textView
{
    ...
    UIFont *font = HMSFOND(MHPxConvertPt(9.0f));
    UIColor *color = textView.attributedText.length<=150 ? [UIColor lightGrayColor] : [UIColor orangeColor];
...
- (void) _send
{
    ...
    if (self.textView.attributedText.length >MHCommentMaxWords) {
        [SVProgressHUD showErrorWithStatus:@"回复内容超过上限"];
        return;
    }
    // 代理回调
