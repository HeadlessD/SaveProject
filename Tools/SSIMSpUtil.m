...
+ (NSString *)encryptPassword:(NSString *)password userName:(NSString *)userName{
    return [[NSString stringWithFormat:@"%@{%@}", password, userName] MD5Hash];
    //    return nil;
}
...
+(CGSize)getChatImageSize:(UIImage *)image
{
    ...
    if (imgW==0 || imgH == 0 || image == nil) {
        imgW = KScreenWidth*0.45;
        imgH = KScreenHeight*0.3;
    }
    
    CGFloat bgW = KScreenWidth*0.4;
    CGFloat bgH = KScreenWidth*0.4;
    ...
    if (imgW > imgH) {//宽图
        ...
        if (imgW>=bgW) {
            scale = imgH/imgW;
            bgH = scale*bgW;
            if (bgH<=KScreenWidth*0.2) {
                bgH = KScreenWidth*0.2;
            }
        }else{
            scale = imgH/imgW;
            bgH = scale*bgW;
        ...
        if (imgH>=bgH) {
            bgW = bgH*scale;
            if (bgW<=KScreenWidth*0.25) {
                bgW = KScreenWidth*0.25;
            }
...
+ (void )LeftBubbleImage:(UIImageView *)imageView
{
    /*
     ...
     UIImage *img = [SSIMSpUtil stretchImage:IMG2XFROMBUNDLE(@"bubble_mask_l.png") withLeftCapWidth:18 topCapHeight:18 capInsets:UIEdgeInsetsMake(18, 18, 17, 26) resizingMode:UIImageResizingModeStretch];
...
+ (void )RightBubbleImage:(UIImageView *)imageView
{
    /*
     ...
     {
     CALayer *mask = [CALayer layer];
     UIImage *img = [SSIMSpUtil stretchImage:IMG2XFROMBUNDLE(@"bubble_mask_r.png") withLeftCapWidth:18 topCapHeight:18 capInsets:UIEdgeInsetsMake(18, 18, 17, 26) resizingMode:UIImageResizingModeStretch];
     UIImage *newImg = [SSIMSpUtil scaleFromImage:img toSize:CGSizeMake(imageView.bounds.size.width, imageView.bounds.size.height)];
     mask.contents = (id)[newImg CGImage];
     mask.frame = CGRectMake(0, 0, imageView.bounds.size.width, imageView.bounds.size.height);
     imageView.layer.mask = mask;
...
+(void)setBubbleImageButton:(UIButton*)btn isLeft:(BOOL)left
{
    UIImage *img;
    if (left) {
        img=IMG2XFROMBUNDLE(@"image_mask.png");
    }else{
        img=IMG2XFROMBUNDLE(@"image_mask_sender.png");
    }
    CALayer *mask = [CALayer layer];
...
+ (UIBarButtonItem *)backBarButtonItemWithTitle:(NSString *)title Traget:(id)traget sel:(SEL)sel arrow:(BOOL)arrow;
{
    ...
    if(arrow)
    {
        [btn setImage:IMG2XFROMBUNDLE(@"back.png") forState:UIControlStateNormal];
    }
